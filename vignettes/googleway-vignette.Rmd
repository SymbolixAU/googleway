...
Having retreived the polyline, you can decode it into latitude and longitude coordinates using `decode_pl()`.
...
You can specify a column in the data.frame to use to populate a popup `info_window` that will be displayed when clicking on a maker. The info window can dispay any valid HTML, as demonstrated in [this Stack Overflow answer](http://stackoverflow.com/a/39339464/5977215).
...
Polylines in Google Maps are formed from a set of latitude/longitude cooridnates, encoded using a lossy compression algorithm (https://developers.google.com/maps/documentation/utilities/polylinealgorithm) that allows you to store the series of coordinates as a single string. 
