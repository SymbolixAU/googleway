...
Googleway provides access to 7 Google Maps APIs through specified functions, and the ability to plot a Google Map overlayed with markers, circles, shapes (polygons), lines (polylines) and heatmaps. You can also overlay traffic information, transit and cycling routes.

The 7 API functions are
...
Having retreived the polyline, we can decode it using `decode_pl()`
...
For example, a query  for "restaurantes in Melbourne"" (a query will return 20 results)
