...
#'
#' @param place_input \code{string} The text string on which to search. The Place Autocomplete service will return candidate matches based on this string and order results based on their perceived relevance.
#' @param location \code{numeric} vector of latitude/longitude coordinates (in that order). he point around which you wish to retrieve place information
#' @param radius \code{numeric} The distance (in meters) within which to return place results. Note that setting a radius biases results to the indicated area, but may not fully restrict results to the specified area
...
#' @param place_type \code{string} Restricts the results to places matching the specified type. Only one type may be specified (if more than one type is provided, all types following the first entry are ignored). For a list of valid types, please visit \url{https://developers.google.com/places/web-service/autocomplete#place_types}
#' @param components \code{string} of length 1 Identifies a grouping of places to which you would like to restrict your results. Currently, you can use components to filter by country only. The country must be passed as a two character, ISO 3166-1 Alpha-2 compatible country code. For example: components=country:fr would restrict your results to places within France.
...
google_place_autocomplete <- function(place_input,
  ...
  #     stop("components must be a data.frame with two columns, 'component' and 'value'")
  #
  #   if(!tolower(names(df)) %in% c("component","value"))
  #     stop("components must be a data.frame with two columns, 'component' and 'value'")
  # }
  #
