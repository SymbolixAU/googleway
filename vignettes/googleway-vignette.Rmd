...
- use cases - multiple calls / loops
- sf objects
---
...
```
legendOpts <- list(position = "TOP_RIGHT", css = "max-height: 100px")
...
```
![Legend Options](./img/polygon_legend_custom.png)

If you are displaying two legends, one for `stroke_colour` and one for `fill_colour`, you can specify different options fo rthe different colour attributes
...
google_map(key = "your_api_key") %>%
  add_rectangles(data = df, north = 'north', south = 'south',
                 east = 'east', west = 'west')
```
![Recctangles](./img/rectangles.png)
...
=======
One of the more powerful features of `googleway` is the ability to return data back to R from Shiny. 
>>>>>>> de54dac30ab1e5a956f1b64594e1b525d81d8337
