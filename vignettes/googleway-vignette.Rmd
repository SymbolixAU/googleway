...
As `google_map()` is a `HTMLWidget`, in inherently works in *Shiny*. As with all shiny apps the two functions you need to include are

* `renderGoogle_map()` (in the UI)
* `google_mapOutput()` (in the server)

The simplest app can be build with
...
You can use all the standard `add_*` functions that have already been specified to add various shapes and layers. But there's also varous `clear_*` and `update_*` functions that let you update those shapes and layers dynamically with the shiny app. 

The `clear_*` functions are designed to remove objects from the app. The `update_*` functions are designed to update those objects that are already on the map. 
