...
Plus the helper fucntions, `encode_pl()` and `decode_pl()` for encoding and decoding polylines.
...
For a **nearby** search your are required to porvide a `location` as a pair of latitude/longitude coordinates. You can refine your search by providing a search string and / or a radius.
...
![Traffic](./img/add_traffic.png)


![Transit](./img/add_transit.png)


![Bicycling](./img/add_bicycling.png)

You can also include a search box in your map by using the argument `search_box = TRUE`
...
```
google_map(data = tram_stops, key = map_key) %>%
  add_circles(lat = "stop_lat", lon = "stop_lon", radius = 50, fill_colour = "#00FF00")
```

![Route 35 tram stops - circles](./img/add_circles.png)
...
You can create a heatmap using Google's [Heatlayer](https://developers.google.com/maps/documentation/javascript/examples/layer-heatmap), which in googleway is called with (not surprisingly) `add_heatmap()`
...
There are a few options you can configure to change how the heatmap is plotted, for example changing the colours, and the 'weight' associated with each point in the data set
...
To draw a line on a map you use the `add_polylines()` function. This function takes a data.frame with at least one column of data containing the polylines, or two columns containing the series of lat/lon coordinates that make up a polyline. 
...
* Rows of nested polylines in a list column. Each row represents a polygon, and each polylne in the list represents the paths that make up the polygon.


These examples show a single polygon made up of three paths. 
...
pl_other <- encode_pl(c(21,28,22), c(-63, -58, -59))
...
There are various other arguments you can use in the `add_*` functions, in particular

* info_window - displays an information window when clicking on the map object
* mouse_over - displays an information window when hovering over the map object
* mouse_over_group - highlights a group of shapes when overing over a map object

The following code and three images show polygons defined with an `id`, and the `mouse_over_group` using this `id` to highlighted associated polygons within the same group


```
pl1 <- encode_pl(lat = c(25.774, 18.466,32.321),
                      lon = c(-80.190, -66.118, -64.757))

pl2 <- encode_pl(lat = c(28.745, 29.570, 27.339),
                      lon = c(-70.579, -67.514, -66.668))

pl3 <- encode_pl(c(21,28,22), c(-63, -58, -59))

pl4 <- encode_pl(c(21, 28, 24), c(-63, -51, -53))

pl5 <- encode_pl(c(29, 27, 30), c(-71, -73, -75))

df <- data.frame(id = c(1,1,2,1, 2),
                 polyline = c(pl1, pl2, pl3, pl4, pl5),
                 stringsAsFactors = FALSE)

google_map(key = map_key, height = 800) %>%
  add_polygons(data = df, polyline = "polyline", id = 'id', mouse_over_group = 'id')
  
```

![Mouse Over Group](./img/mouse_over_group_off.png)

![Mouse Over Group 1](./img/mouse_over_group_on1.png)

![Mouse Over Group 2](./img/mouse_over_group_on2.png)
...
As `google_map()` is a `HTMLWidget`, it inherently works in *Shiny*. As with all shiny apps the two functions you need to include are
...
You can use all the standard `add_*` functions that have already been discussed to add various shapes and layers. But there's also varous `clear_*` and `update_*` functions that let you update those shapes and layers dynamically within the shiny app. 
...
The exception to this is `update_heatmap()`, which allows you to both add and remove points from the heat layer
...
Note that the `update_*` functions can't be used to add extra objects, they can only update objects that already exist.
